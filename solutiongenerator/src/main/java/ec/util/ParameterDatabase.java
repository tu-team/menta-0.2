...
public class ParameterDatabase extends Properties implements Serializable {
    ...
    public Object getInstanceForParameter(Parameter parameter, Parameter defaultParameter, Class mustCastTosuperclass) throws ParamClassLoadException {
        ...
        try {
            Class c;
            if (get(p).toString().contains(IndividualEncoder.mentaPrefix())) {
                return IndividualEncoder.createIndividual(get(p).toString());
            } else {
                c = Class.forName(get(p));
            }
    ...
    public Object getInstanceForParameterEq(Parameter parameter,
        ...
        try {
            Class c;
            if (p.toString().contains(IndividualEncoder.mentaPrefix())) {
                return IndividualEncoder.createIndividual(p.toString());
            } else {
                c = Class.forName(get(p));
            }
            if (!mustCastTosuperclass.isAssignableFrom(c))
...
*/
     ...
     */
    public ParameterDatabase(java.util.Dictionary map) throws Exception, IOException {
        this();
        LOG.debug(" public constructor ParameterDatabase({})", map);
        java.util.Enumeration keys = map.keys();
        LOG.debug(" keys {}", keys);
//            try {
//
//            } catch (Exception e) {
//                throw new Exception("fuck line out");
//            }
        while (keys.hasMoreElements()) {
            Object obj = keys.nextElement();
            LOG.debug(" obj {}", obj);
            set(new Parameter("" + obj), "" + map.get(obj));
            LOG.debug(" map.get(obj) {}", map.get(obj));
        }
...
//            if (new File(s).isAbsolute()) // it's an absolute file definition
            LOG.debug("1 parents {} s {}", parents, s);
            File f = new File(s);
            if (!f.exists()) {
...
//            else throw new FileNotFoundException("Attempt to load a relative file, but there's no parent file: " + s);
                LOG.debug("2 parents {} s {}", parents, s);
            }
        }
    }
