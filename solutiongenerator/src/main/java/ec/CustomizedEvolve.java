...
import ec.util.*;
import menta.model.howto.ActionClass;
...
public class CustomizedEvolve {
    ...
    static Logger LOG = LoggerFactory.getLogger("CustomizedEvolve");

    /**
     * Loads a ParameterDatabase from checkpoint if "-params" is in the command-line arguments.
     */
    public static ParameterDatabase loadParameterDatabase(Dictionary<String, String> args) {
        ParameterDatabase parameters = null;
        LOG.debug("parameters {}", parameters);

        try {
            parameters = new ParameterDatabase(args);
        } catch (Exception e) {
            Output.initialError(
                    "An exception was generated upon reading parameters: see line 194 (loadParameterDatabase) " + e.getMessage());
        }
        if (parameters == null)
...
*/
    ...
    public static void evaluate(Dictionary<String, String> args) {
        EvolutionState state;
        ParameterDatabase parameters;

        // if we're loading from checkpoint, let's finish out the most recent job
        state = possiblyRestoreFromCheckpoint(dictionaryToStringArray(args));
        ...
        if (state != null)  // loaded from checkpoint
        {
            ...
            try {
                ...
                args = stringArrayToDictionary(state.runtimeArguments);                          // restore runtime arguments from checkpoint
        ...
        for (int job = currentJob; job < numJobs; job++) {
            try {
                ...
                state.runtimeArguments = dictionaryToStringArray(args);                              // stick the runtime arguments in our storage
    ...
    public static String[] dictionaryToStringArray(Dictionary<String, String> dict) {
        String[] str = new String[dict.size() * 2];
        int i = 0;
    ...
    public static Dictionary<String, String> stringArrayToDictionary(String[] str) {
        Dictionary<String, String> dict = new Hashtable<String, String>();
        int i = 0;
        while (i < str.length) {
            dict.put(str[i], str[i + 1]);
            i = i + 2;
        }
        return dict;
    }
